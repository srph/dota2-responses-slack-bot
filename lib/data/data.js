...
(async function() {
  try {
    ...
    for ( let i = 0; i < announcers.length; i++ ) {
      ...
      const announcerWiki = `${RESPONSES_BASE_URL}/${toURIFormat(announcer)}`;
      console.log(`querying: ${announcerName}`);

      try {
        const page$ = await axios.get(announcerWiki)
        const body = page$.data;
        const $ = cheerio.load(body);
        
        // All responses have the `sm2_button` class
        // which styles the link to the audio.
        const responses = $('li .sm2_button')
          // We want to get the parent (`li` in this case)
          // because it contains all the data we need.
          // (e.g., exact response, audio url)
          ...
          .filter(response => response.text && response.text.length);
